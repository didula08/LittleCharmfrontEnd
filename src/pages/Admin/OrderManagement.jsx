          ...
          <div className="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-fadeIn">
            ...
            <div className="bg-primary p-6 text-white flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-lite/60 font-mono">ID: #{selectedOrder.orderId}</p>
              </div>
              <button 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <p className="font-bold text-primary text-lg">{selectedOrder.name}</p>
                    <p className="text-sm text-gray-500 flex items-center gap-2"><FaEnvelope className="text-accent" /> {selectedOrder.email}</p>
                    <p className="text-sm text-gray-500 flex items-center gap-2"><FaPhoneAlt className="text-accent" /> {selectedOrder.phone}</p>
                    <p className="text-sm text-gray-500 flex items-start gap-2 pt-1"><FaMapMarkerAlt className="text-accent mt-1 shrink-0" /> <span className="leading-tight">{selectedOrder.address}</span></p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="font-bold text-primary">{new Date(selectedOrder.date).toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 mb-1">Status</p>
                      <select 
                        className={`text-xs font-bold px-3 py-1.5 rounded-full outline-none border-none cursor-pointer ${getStatusColor(selectedOrder.status)}`}
                    ...
                    <tbody className="divide-y divide-black/5">
                      {selectedOrder.products.map((item, idx) => (
                        <tr key={idx} className="text-sm">
                          ...
                          <td className="px-4 py-3 text-right text-gray-500">LKR {item.productInfo.price?.toLocaleString()}</td>
                          <td className="px-4 py-3 text-right font-bold text-primary">LKR {(item.productInfo.price * item.quantity).toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot className="bg-primary/5 font-bold">
                       <tr>
                         <td colSpan="3" className="px-4 py-3 text-right text-primary uppercase text-[10px]">Total Amount</td>
                         <td className="px-4 py-3 text-right text-secondary text-lg">LKR {selectedOrder.total?.toLocaleString()}</td>
                       </tr>
                    </tfoot>
                  </table>
                </div>
            ...
            <div className="p-6 bg-gray-50 border-t border-black/5 flex justify-end gap-3">
              <button 
                ...
                className="flex items-center gap-2 px-6 py-2 rounded-xl border border-black/10 text-gray-600 font-bold text-sm hover:bg-white transition-all cursor-pointer"
              >
                <FaPrint /> Print Invoice
              </button>
              <button 
                onClick={() => setSelectedOrder(null)}
                className="px-6 py-2 rounded-xl bg-primary text-white font-bold text-sm hover:translate-y-[-2px] transition-all shadow-lg cursor-pointer"
              >
                Close
              </button>
            </div>
