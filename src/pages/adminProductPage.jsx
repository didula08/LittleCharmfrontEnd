  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex justify-between items-end">
        ...
        <Link
          to="/admin/addProduct"
          className="flex items-center gap-2 bg-secondary text-lite px-6 py-3 rounded-2xl font-bold hover:translate-y-[-2px] transition-all shadow-lg active:scale-95"
        >
      ...
      {/* Control Bar */}
      <div className="flex flex-col md:flex-row gap-4 items-center bg-white p-4 rounded-2xl border border-black/5 shadow-sm">
        <div className="flex-1 flex items-center gap-4 w-full">
           ...
           <select 
             className="bg-gray-50 border border-black/5 px-4 py-2.5 rounded-xl outline-none text-sm font-bold text-gray-500 cursor-pointer focus:ring-2 focus:ring-accent/20"
             value={statusFilter}
        ...
        {selectedProducts.length > 0 && (
          <div className="flex items-center gap-2 px-4 py-2 bg-accent/5 rounded-xl border border-accent/10 whitespace-nowrap animate-fadeIn">
          ...
          <thead>
            <tr className="bg-gray-50/50 border-b border-black/5">
              <th className="px-6 py-4 w-12">
                <button 
                  onClick={selectAll}
                  className="w-5 h-5 flex items-center justify-center rounded border border-gray-300 bg-white text-accent transition-colors"
                >
                ...
                <tr key={item.productId} className={`hover:bg-gray-50/50 transition-colors group ${selectedProducts.includes(item.productId) ? 'bg-secondary/5' : ''}`}>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 rounded-2xl bg-gray-100 overflow-hidden border border-black/5 shrink-0 group-hover:scale-105 transition-transform duration-500 shadow-sm">
                        {item.image?.[0] ? (
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-primary truncate max-w-[200px] leading-tight text-sm">{item.name}</p>
                        <p className="text-[10px] text-gray-400 uppercase font-mono tracking-tighter mt-1">ID: {item.productId}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <p className="font-extrabold text-primary text-sm">${item.price?.toFixed(2)}</p>
                    <p className="text-[10px] text-gray-400 line-through opacity-50">${item.labelledPrice?.toFixed(2)}</p>
                  </td>
                  <td className="px-6 py-4 text-right">
                  ...
                  <td className="px-6 py-4 text-center">
                    <button 
                      ...
                      className={`inline-flex px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer shadow-sm active:scale-95 ${
                        item.isAvailable 
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-1">
                      <button 
                        ...
                        className="p-2.5 text-blue-500 hover:bg-blue-50 rounded-xl transition-all cursor-pointer active:scale-90"
                        title="Edit Details"
                      >
                        <FaEdit />
                      </button>
                      <button 
                        ...
                        className="p-2.5 text-red-400 hover:bg-red-50 rounded-xl transition-all cursor-pointer active:scale-90"
              ...
              <tr>
                <td colSpan="6" className="px-6 py-20 text-center flex flex-col items-center justify-center gap-4">
                  ...
                  <div className="text-gray-400">
                    ...
                    <button onClick={() => {setSearchTerm(''); setStatusFilter('all');}} className="text-accent text-xs hover:underline mt-1 font-bold">Clear all filters</button>
                  </div>
                </td>
              </tr>
            )}
