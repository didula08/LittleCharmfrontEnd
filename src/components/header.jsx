...
export default function Header() {
  ...
  return (
    <>
      {/* Mobile Menu Sidebar - Moved out of <header> to avoid z-index and backdrop-filter issues */}
      <div className={`fixed inset-0 z-[100] md:hidden transition-all duration-300 ${isMenuOpen ? "visible" : "invisible"}`}>
        ...
        {/* Sidebar Content */}
        <div className={`absolute top-0 left-0 h-full w-[300px] bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
          <div className="flex items-center justify-between p-6 border-b border-black/5 bg-gray-50/50">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="font-bold text-xl text-primary tracking-tight">LittleCharm</span>
            </div>
            <button onClick={() => setIsMenuOpen(false)} className="p-2 text-primary/40 hover:text-primary hover:bg-white rounded-full transition-all">
          ...
          <nav className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-2">
              ...
              return (
                <Link
                  ...
                  className={`text-lg font-bold px-5 py-4 rounded-2xl transition-all flex items-center justify-between group ${
                    isActive 
                      ? "bg-secondary text-lite shadow-lg shadow-secondary/20" 
                      : "text-primary/70 hover:bg-gray-50 hover:text-primary"
                  }`}
                >
                  {link.name}
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-4 h-4 transition-transform ${isActive ? "translate-x-0 opacity-100" : "-translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100"}`}>
            ...
            <div className="h-px bg-black/5 my-6 mx-4"></div>
            
            <Link
              to="/login"
              onClick={() => setIsMenuOpen(false)}
              className="text-lg font-bold px-5 py-4 rounded-2xl text-primary/70 hover:bg-gray-50 hover:text-primary flex items-center gap-4"
            >
              <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-primary/50">
          ...
          <div className="p-8 border-t border-black/5 bg-gray-50/30">
            <p className="text-[10px] text-primary/30 font-black uppercase tracking-[0.2em] mb-1">E-Commerce Boutique</p>
            <p className="text-xs text-primary/40 font-medium whitespace-nowrap">© 2026 LittleCharm. Handcrafted with care.</p>
          </div>
        </div>
      </div>
